...
function ModalCreate() {
  ...
  const { setShowModalCreateProduct, uploadImage, uploadImageList, createProductFunc, showModalCreateProduct, setShowModalEdit, showModalEdit, productState: { product } } = useContext(ProductContext)
  ...
  var nameValue = Form.useWatch("name", form);
  ...
  const handleSubmitCreate = async () => {
    ...
    if(product_quantity > 500) {
      toast.error("Số Lượng Phải Nhỏ Hơn 500")
      return
    } 
    if (!isNumeric(product_price)) {    
      toast.error("Gía Bạn Nhập Không Hợp Lệ")
       return
    }
  ...
  const handleCreateImages = () => {
    window.location.href="https://design-for-shop-o1z9.vercel.app/"
  }
        ...
        <MDBox pt={2} px={2}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={4}>
              ...
              <Button onClick={handleSubmitCreate} style={{backgroundColor: '#BFDBFD', marginRight: '20px'}}>Tạo </Button>
              <Button onClick={handleCreateImages}style={{backgroundColor: '#0D2D53', color:'#fff'}}>Thiết Kế </Button>
            </Grid>
